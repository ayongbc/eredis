...
-export([get_master/1, get_master/2, get_current_sentinel/1]).
...
-define(EREDIS_SENTINEL_ETS, eredis_sentinel_ets).
...
stop(MasterName) ->
    Pid = ets:lookup_element(?EREDIS_SENTINEL_ETS, MasterName, 2),
    ets:delete(?EREDIS_SENTINEL_ETS,MasterName),
    gen_server:call(Pid, stop).

get_master(MasterName) ->
    Pid = ets:lookup_element(?EREDIS_SENTINEL_ETS, MasterName, 2),
    get_master(Pid, false).

get_master(MasterName, Notify) when is_atom(MasterName), is_boolean(Notify) ->
    Pid = ets:lookup_element(?EREDIS_SENTINEL_ETS, MasterName, 2),
    gen_server:call(Pid, #get_master_req{master = MasterName, notify = Notify}).

get_current_sentinel(MasterName) ->
    Pid = ets:lookup_element(?EREDIS_SENTINEL_ETS, MasterName, 2),
    gen_server:call(Pid, get_current_sentinel).

%%% GenServer ---------------------------------------------------------

init({MasterName, Sentinels}) ->
    create_ets(),
    ets:insert(?EREDIS_SENTINEL_ETS, {MasterName, self()}),
...
create_ets() ->
    case ets:info(?EREDIS_SENTINEL_ETS) of
        undefined ->
            ets:new(?EREDIS_SENTINEL_ETS, [set, named_table, public, {write_concurrency, true}, {read_concurrency, true}]);
        _InfoList ->
            ?EREDIS_SENTINEL_ETS
    end.
